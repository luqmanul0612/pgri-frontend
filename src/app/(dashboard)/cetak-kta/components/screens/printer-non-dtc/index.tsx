...
const PrinterNonDtc: FC<PrinterDtcProps> = ({ onBack }) => {
  ...
  const transformMemberToKtaData = (member: IMember): CetakKtaTableData => ({
    id: member.id,
    npa: member.npa_number || "",
    nik: member.nik || "",
    namaAnggota: member.name || "",
    tempatLahir: member.birth_place || "",
    tanggalLahir: member.dob || "",
  ...
  const fetchData = async (isPageChange = false) => {
    ...
    try {
      ...
      const totalPages =
        ...
        Math.ceil((result.data?.total || 0) / currentParams.limit);
      const totalItems = result.data?.total || pagination?.total_items || 0;
  ...
  const handleCetak = async () => {
    ...
    try {
      ...
      toast.success("KTA berhasil dicetak");
    } catch (error) {
  ...
  const columnsNonDtc = createColumns(
    data,
    handleSelectAll,
    handleSelectRow,
    Checkbox,
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-[10px] p-3 outline outline-1 -outline-offset-1 transition-colors ${
              data.filter((row) => row.selected).length > 0 && !isLoading
                ? "cursor-pointer bg-primary text-white outline-primary hover:bg-primary/90"
                : "cursor-not-allowed bg-transparent text-primary opacity-50 outline-primary"
            }`}
          >
